    ...
    for i in range(n):
        ...
        if problem_type == 0:
            ...
            while attempts < 50:
                ...
                paul_time = random.randint(10, 50)
                katya_time = random.randint(1, 5)
                david_time = katya_time + random.randint(8, 20)
                joe_time = david_time - random.randint(5, 20)
                
                # Ensure all times are positive and unique
                times = [paul_time, katya_time, david_time, joe_time]
                if len(set(times)) == 4 and min(times) > 0:
                    
                    # Determine who finishes fourth
                    sorted_times = sorted(times)
                    fourth_time = sorted_times[3]
                    ...
                    fourth_person = time_to_person[fourth_time]
                    
                    # Calculate relationship values
                    katya_before_david = david_time - katya_time
                    joe_before_david = david_time - joe_time
                    
                    # Create the question text
                    q_text = f"At the end of a race, Paul finishes {paul_time} seconds after Nancy. "
                    q_text += f"Katya finishes {katya_time} second after Nancy but {katya_before_david} seconds before David, "
                    q_text += f"and Joe finishes {joe_before_david} seconds before David. Who finishes fourth?"
                    
                    # Generate answer choices - only names
                    all_names = ["Paul", "Katya", "David", "Joe"]
                    
                    # Remove the correct answer from the pool for distractors
                    distractors = [name for name in all_names if name != fourth_person]
                    random.shuffle(distractors)
                    
                    # Build choices with correct answer + 3 distractors
                    choices = [fourth_person] + distractors[:3]
                    
                    # Make sure we have all 4 names
                    if len(set(choices)) == 4:
                        random.shuffle(choices)
                        correct_index = choices.index(fourth_person)
                        
                        # Break out of the loop - we have a valid problem
                        break
            else:
                # Fallback if no valid problem found
                q_text = "At the end of a race, Paul finishes 20 seconds after Nancy. Katya finishes 1 second after Nancy but 13 seconds before David, and Joe finishes 15 seconds before David. Who finishes fourth?"
                choices = ["Paul", "Katya", "David", "Joe"]
                random.shuffle(choices)
                correct_index = choices.index("David")
            
            # Create the problem tuple that matches your expected format
            # This should match what your other problem types return
            problem = (q_text, choices, correct_index)
...
/* Bronze Crown - 70-79% */
.crown-bronze .crown-base { fill: #cd7f32; }
.crown-bronze .crown-point { fill: #cd7f32; }
.crown-bronze .crown-gem { fill: #8b5a2b; }
.crown-bronze .crown-highlight { fill: #e8a85c; }
.crown-bronze .crown-dark { fill: #8b5a2b; }
.crown-bronze .crown-filigree { stroke: #a0672a; }
.crown-bronze .crown-shine { fill: rgba(255,255,255,0.3); }

/* Silver Crown - 80-89% */
.crown-silver .crown-base { fill: #c0c0c0; }
.crown-silver .crown-point { fill: #c0c0c0; }
.crown-silver .crown-gem { fill: #4a6fa5; }
.crown-silver .crown-highlight { fill: #f0f0f0; }
.crown-silver .crown-dark { fill: #8a8a8a; }
.crown-silver .crown-filigree { stroke: #a0a0a0; }
.crown-silver .crown-shine { fill: rgba(255,255,255,0.4); }

/* Gold Crown - 90-99% */
.crown-gold .crown-base { fill: #ffd700; }
.crown-gold .crown-point { fill: #ffd700; }
.crown-gold .crown-gem { fill: #dc143c; }
.crown-gold .crown-highlight { fill: #fff8dc; }
.crown-gold .crown-dark { fill: #c8960a; }
.crown-gold .crown-filigree { stroke: #d4a017; }
.crown-gold .crown-shine { fill: rgba(255,255,255,0.5); }

/* Diamond Crown - 100% */
.crown-diamond .crown-base { fill: #ffd700; }
.crown-diamond .crown-point { fill: #ffd700; }
.crown-diamond .crown-gem { fill: #b9f2ff; }
.crown-diamond .crown-highlight { fill: #fff8dc; }
.crown-diamond .crown-dark { fill: #c8960a; }
.crown-diamond .crown-filigree { stroke: #d4a017; }
.crown-diamond .crown-shine { fill: rgba(255,255,255,0.6); }
.crown-diamond .crown-diamond-gem { fill: #e0f7fa; }

/* Section score crowns - smaller */
.section-crown {
    display: inline-flex;
    align-items: center;
    gap: 8px;
    ...
    <div style="background: #e8f0fe; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
        <div class="score-display">
            ...
            <div class="crown-container">
                {% if percent >= 100 %}
                    ...
                    <svg class="crown-svg crown-diamond" viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
                        <defs>
                            ...
                            <linearGradient id="crownShine1" x1="0%" y1="0%" x2="100%" y2="100%">
                        ...
                        <!-- Shiny reflection on points -->
                        <polygon points="50,25 60,15 70,25" fill="url(#crownShine1)"/>
                        <!-- Main diamond gems -->
                        ...
                        <path class="sparkle-star" d="M42,40 L40,37 M42,40 L44,37" stroke="#fff8dc" stroke-width="1" fill="none" style="animation-delay:1.5s"/>
                        <path class="sparkle-star" d="M78,40 L76,37 M78,40 L80,37" stroke="#fff8dc" stroke-width="1" fill="none" style="animation-delay:0.8s"/>
                        <!-- Decorative dots -->
                ...
                {% elif percent >= 90 %}
                    <!-- Gold Crown - 90-99% -->
                    <svg class="crown-svg crown-gold" viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
                        <defs>
                ...
                {% elif percent >= 80 %}
                    <!-- Silver Crown - 80-89% -->
                    <svg class="crown-svg crown-silver" viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
                        <defs>
                ...
                {% elif percent >= 70 %}
                    <!-- Bronze Crown - 70-79% -->
                    <svg class="crown-svg crown-bronze" viewBox="0 0 120 120" xmlns="http://www.w3.org/2000/svg">
                        <defs>
        ...
        <!-- Section Scores with small shiny crowns -->
        <div class="section-score-display" style="margin-top: 18px; padding-top: 18px; border-top: 2px solid #dee2e6;">
            <div style="display:flex; align-items:center; gap:8px; background: #f8f9fa; padding: 6px 14px; border-radius: 20px;">
                ...
                {% if reading_score >= 90 %}
                    <svg width="32" height="32" viewBox="0 0 60 60" style="display:inline-block; filter: drop-shadow(0 2px 8px rgba(255,215,0,0.3));">
                        <polygon points="9,42 12,15 18,33 30,8 42,33 48,15 51,42" fill="#ffd700" stroke="#c8960a" stroke-width="1"/>
                        <circle cx="12" cy="20" r="4" fill="#dc143c"/>
                        <circle cx="30" cy="12" r="5.5" fill="#dc143c"/>
                        <circle cx="48" cy="20" r="4" fill="#dc143c"/>
                        <ellipse cx="29" cy="10" rx="2" ry="1.5" fill="white" opacity="0.5"/>
                    </svg>
                {% elif reading_score >= 80 %}
                    <svg width="32" height="32" viewBox="0 0 60 60" style="display:inline-block; filter: drop-shadow(0 2px 8px rgba(192,192,192,0.3));">
                        <polygon points="9,42 12,15 18,33 30,8 42,33 48,15 51,42" fill="#c0c0c0" stroke="#8a8a8a" stroke-width="1"/>
                        <circle cx="12" cy="20" r="4" fill="#4a6fa5"/>
                        <circle cx="30" cy="12" r="5.5" fill="#4a6fa5"/>
                        <circle cx="48" cy="20" r="4" fill="#4a6fa5"/>
                        <ellipse cx="29" cy="10" rx="2" ry="1.5" fill="white" opacity="0.5"/>
                    </svg>
                {% elif reading_score >= 70 %}
                    <svg width="32" height="32" viewBox="0 0 60 60" style="display:inline-block; filter: drop-shadow(0 2px 8px rgba(205,127,50,0.3));">
                        <polygon points="9,42 12,15 18,33 30,8 42,33 48,15 51,42" fill="#cd7f32" stroke="#8b5a2b" stroke-width="1"/>
                        <circle cx="12" cy="20" r="4" fill="#8b5a2b"/>
                        <circle cx="30" cy="12" r="5.5" fill="#8b5a2b"/>
                        <circle cx="48" cy="20" r="4" fill="#8b5a2b"/>
                        <ellipse cx="29" cy="10" rx="2" ry="1.5" fill="white" opacity="0.4"/>
                    </svg>
                {% endif %}
            </div>
            <div style="display:flex; align-items:center; gap:8px; background: #f8f9fa; padding: 6px 14px; border-radius: 20px;">
