...
if ON_RENDER:
    ...
    def get_all_users():
        ...
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("SELECT username, is_admin, created_at FROM users ORDER BY created_at")
        users = cur.fetchall()
        cur.close()
        conn.close()
...
def sample_test():
    ...
    if request.method == 'GET':
        ...
        # CRITICAL: Store EVERYTHING needed in session
        ...
        # Force session to save (important for some deployments)
    ...
    # ============================================================
        ...
        # CRITICAL: Get data from session
        problems = session.get('sample_test_problems', [])
        corrects = session.get('sample_test_correct_indices', [])
        
        # DEBUG: Print to Render logs
        print(f"Processing POST - Problems count: {len(problems)}")
        print(f"Processing POST - Corrects count: {len(corrects)}")
        ...
        print(f"SCORES - Reading: {reading_percent}%, Verbal: {verbal_percent}%, Math: {math_percent}%, Total: {total_percent}%")
        
        # Save to CSV (or PostgreSQL)
