...
{% if total_attempts == 0 %}
    <div style="text-align: center; padding: 40px;">
        <h2>No Data Yet</h2>
        <p>You haven't taken any sample tests yet.</p>
        <p>Click "Take Sample Test" on the home page to get started!</p>
...
{% else %}
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="stat-label">Avg of Last 5</div>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ attempt.reading_type }}</td>
                <td><strong>{{ attempt.total_score }}%</strong></td>
                <td>{{ attempt.reading_score }}%</td>
                <td>{{ attempt.verbal_score }}%</td>
                <td>{{ attempt.math_score }}%</td>
            </tr>
            {% endfor %}
        </tbody>
    </table>
...
<div id="resetModal" class="modal">
    <div class="modal-content">
        ...
        <input type="password" id="resetPassword" placeholder="Enter password (1234)">
...
button.success:hover { background: #1b4d3e; }
input { padding: 8px 12px; margin: 5px; border: 2px solid #ccd; border-radius: 8px; font-size: 14px; }
table { width: 100%; border-collapse: collapse; margin: 20px 0; }
...
<div class="user-form">
    ...
    <form id="createUserForm">
        ...
        <button type="submit" class="success" id="createBtn">Create User</button>
    </form>
...
async function createUser(event) {
    ...
    try {
        ...
        if (!contentType || !contentType.includes('application/json')) {
            ...
            showMessage('Server error. Please check logs.', 'error');
            return;
        }
...
def generate():
    ...
    if section == 'reading':
        ...
        try:
            ...
            print(f"Error generating reading problems: {e}")
            # Fallback reading problem
            problems = [
                ("The quick brown fox jumps over the lazy dog.", [], -1),
                ("What animal is mentioned in the passage?", ["fox", "dog", "cat", "bird", "rabbit"], 0),
                ("What color is the fox?", ["brown", "red", "gray", "black", "white"], 0),
            ]
    ...
    # ============================================================
        ...
        if section == 'verbal':
            try:
                ...
                print(f"Error generating verbal problems: {e}")
                # Fallback verbal problems
                problems = []
                for i in range(num):
                    problems.append((
                        f"What is a synonym for 'good'?",
                        ["excellent", "bad", "poor", "terrible", "awful"],
                        0
                    ))
        ...
        # ========== QUANTITATIVE SECTION ==========
            ...
            try:
                ...
                print(f"Error generating quant problems: {e}")
                ...
                for i in range(num):
                    ...
                    problems.append((
                        f"What is {a} + {b}?",
                        [str(a+b), str(a+b+1), str(a+b-1), str(a+b+2), str(a+b-2)],
                        0
                    ))
        ...
        for q, choices, correct in problems:
            # Make a copy of choices to avoid modifying original
            choices = list(choices)
            ...
            if len(choices) > 5:
                # Keep the correct answer in the list
                correct_choice = choices[correct] if correct < len(choices) else None
                choices = choices[:5]
                if correct_choice and correct_choice in choices:
                    correct = choices.index(correct_choice)
                else:
                    correct = 0
        ...
        if not normalized or len(normalized) == 0:
            ...
            normalized = [("What is 2 + 2?", ["3", "4", "5", "6", "7"], 1)]
...
def sample_test():
    ...
    if request.method == 'GET':
        ...
        print(f"GET - Generated {len(all_problems)} problems")  # Debug
    ...
    # ============================================================
        ...
        print("=== SAMPLE TEST POST STARTED ===")
        ...
        print(f"Problems in session: {len(problems)}")
        print(f"Correct indices in session: {len(corrects)}")
        print(f"Username: {username}")
        print(f"Reading type: {reading_type}")
        
        # If session data is missing, redirect back to generate a new test
        if not problems or len(problems) == 0:
            print("ERROR: No problems found in session, redirecting to GET")
        ...
        for i in range(len(problems)):
            ...
            # Skip passage display items (corrects[i] == -1)
            if corrects[i] == -1:
                results.append(True)
                continue
            ...
            # Grade by section (first 6 questions are Reading)
            if reading_count < 6:
                reading_total += 1
                if ans != -1 and ans == corrects[i]:
                    reading_correct += 1
                    results.append(True)
                    print(f"Q{i} Reading: Correct (answer {ans} == {corrects[i]})")
                else:
                    results.append(False)
                    print(f"Q{i} Reading: Incorrect (answer {ans} != {corrects[i]})")
                reading_count += 1
            
            # Next 20 questions are Verbal
            elif verbal_count < 20:
                verbal_total += 1
                if ans != -1 and ans == corrects[i]:
                    verbal_correct += 1
                    results.append(True)
                    print(f"Q{i} Verbal: Correct")
                else:
                    results.append(False)
                    print(f"Q{i} Verbal: Incorrect")
                verbal_count += 1
            
            # Remaining questions are Math
            else:
                math_total += 1
                if ans != -1 and ans == corrects[i]:
                    math_correct += 1
                    results.append(True)
                    print(f"Q{i} Math: Correct")
                else:
                    results.append(False)
                    print(f"Q{i} Math: Incorrect")
        ...
        print(f"RESULTS:")
        print(f"  Reading: {reading_correct}/{reading_total} = {reading_percent}%")
        print(f"  Verbal: {verbal_correct}/{verbal_total} = {verbal_percent}%")
        print(f"  Math: {math_correct}/{math_total} = {math_percent}%")
        print(f"  Total: {correct_count}/{total_questions} = {total_percent}%")
        ...
        try:
            ...
            print(f"Successfully saved attempt for {username}")
        except Exception as e:
