...
# Initialize problems store
app.config['PROBLEMS_STORE'] = {}
...
def submit_answers():
    ...
    # If already submitted for this problem set, show results
    ...
    # Try to get problems from session first
    problems = session.get('problems', [])
    corrects = session.get('correct_indices', [])
    
    # If session is empty, try the backup store
    if not problems or len(problems) == 0:
        problems_id = session.get('current_problems_id')
        if problems_id and problems_id in app.config.get('PROBLEMS_STORE', {}):
            store_data = app.config['PROBLEMS_STORE'][problems_id]
            problems = store_data['problems']
            corrects = store_data['correct_indices']
            print(f"Submit - Retrieved {len(problems)} problems from backup store")
    
    # Ultimate fallback - should never happen
    if not problems or len(problems) == 0:
        print("ERROR: No problems found in submit route")
        return redirect(url_for('home'))
    ...
    for i in range(len(problems)):
        ...
        # Store the answer
        if session.get('answers') and i < len(session.get('answers', [])):
    ...
    # Also save to backup store if it exists
    problems_id = session.get('current_problems_id')
    if problems_id and problems_id in app.config.get('PROBLEMS_STORE', {}):
        app.config['PROBLEMS_STORE'][problems_id]['results'] = results
        app.config['PROBLEMS_STORE'][problems_id]['submitted_answers'] = submitted_answers
        app.config['PROBLEMS_STORE'][problems_id]['correct_count'] = correct_count
